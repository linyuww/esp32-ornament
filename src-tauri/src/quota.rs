...
fn load_auth() -> Result<AuthTokens, QuotaSnapshot> {
    let path = default_auth_path().ok_or_else(|| {
        QuotaSnapshot::status(
            SnapshotStatus::AuthRequired,
            "无法定位 .codex/auth.json。请先确认 Codex 已登录。",
        )
    })?;
    ...
    if tokens
        ...
        .is_empty()
    {
        return Err(QuotaSnapshot::status(
            SnapshotStatus::AuthRequired,
...
fn fetch_usage(auth: &AuthTokens) -> Result<UsageResponse, QuotaSnapshot> {
    ...
    headers.insert(
        ...
        HeaderValue::from_str(&format!("Bearer {access_token}")).map_err(|_| {
            QuotaSnapshot::status(SnapshotStatus::ParseError, "access_token 无法构造请求头。")
        })?,
    );
    headers.insert(
        "ChatGPT-Account-Id",
        HeaderValue::from_str(account_id).map_err(|_| {
            QuotaSnapshot::status(SnapshotStatus::ParseError, "account_id 无法构造请求头。")
        })?,
    );
    ...
    if status.as_u16() == 401 {
        return Err(QuotaSnapshot::status(
            SnapshotStatus::AuthRequired,
            "ChatGPT 返回 401，access_token 已过期或无效。请在 Codex 里重新登录。",
        ));
    }
...
fn first_limit(rate_limits: &Value, keys: &[&str], nested_keys: &[&str]) -> Option<LimitWindow> {
    keys.iter()
        ...
        .filter_map(|value| parse_limit_entry(value, nested_keys))
        .next()
}
